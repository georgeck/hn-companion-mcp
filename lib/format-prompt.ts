/**
 * Functions for formatting system and user prompts for Claude
 */
...
interface FormattedData {
    systemPrompt: string;
    userPrompt: string;
    commentPathIdMapping: Record<string, string>;
    postTitle: string;
    postId: string;
    commentCount: number;
}
 ...
 * @param formattedComments - Formatted comments
 * @returns User prompt
 */
export function getUserPrompt(postTitle: string, formattedComments: string): string {
...
/**
 * Create a map of comment paths to comment IDs
 * @param comments - Structured comments
 * @returns Map of paths to IDs
 */
export function createCommentPathIdMapping(comments: Comment[]): Record<string, string> {
    const mapping: Record<string, string> = {};
    comments.forEach(comment => {
        mapping[comment.path] = comment.id;
    });
    return mapping;
}

/**
 * Format the comments for Claude
 * @param post - Post data
 * @param comments - Structured comments
 * @returns Formatted data for Claude
 */
export function formatForClaude(post: Post, comments: Comment[]): FormattedData {
    ...
    const userPrompt = getUserPrompt(post.title, formattedComments);
    const commentPathIdMapping = createCommentPathIdMapping(comments);
